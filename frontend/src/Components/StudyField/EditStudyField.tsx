...
export default function EditStudyField() {
  ...
  const [values, setValues] = useState<StudyFieldModel>({
    id:id!,
    name: '',
  } as StudyFieldModel);

  const navigate = useNavigate();
  const [studyField, setStudyField] = useState(false);
  useEffect(() => {
    const fetchData = async () => {
      if(id!=null){
     const response = await StudyFieldService.GetStudyField(id!);
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    const response = await axios.post(
      "https://localhost:7085/api/StudyField",
      model
    );
    setStudyField(true);
    sendToOverview();
...
const handleChange = (
  ...
  return (
    ...
    <h1 style={{ marginLeft: "15px", fontFamily: "Georgia", color: "black" }}>
        {values.id != null ? 'Edit' : 'Add'} Study Field
      </h1>
      <p style={{ marginLeft: "15px", color: "#555", fontSize: "14px" }}>
        Please fill out the form below to {values.id != null ? 'edit' : 'create'} a Study Field.
      </p>
