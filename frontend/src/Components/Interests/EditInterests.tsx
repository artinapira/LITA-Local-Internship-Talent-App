...
export default function EditInterests() {
  ...
  const [values, setValues] = useState<InterestsModel>({
    id:id!,
    name: '',
  } as InterestsModel);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if(id!=null){
     const response = await InterestsService.GetInterests(id!);
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    const response = await axios.post(
      "https://localhost:7085/api/Interests",
      model
    );
    setInterests(true);
    sendToOverview();
...
const handleChange = (
  ...
  return (
    ...
    <h1 style={{ marginLeft: "15px", fontFamily: "Georgia", color: "black" }}>
        {values.id != null ? 'Edit' : 'Add'} Interests
      </h1>
      <p style={{ marginLeft: "15px", color: "#555", fontSize: "14px" }}>
        Please fill out the form below to {values.id != null ? 'edit' : 'create'} a Interests.
      </p>
